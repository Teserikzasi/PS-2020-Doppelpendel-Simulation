...
Die Funktionen zu den \xots s (siehe folgender Abschnitt) befinden sich im Ordner \path{Regelung/x0\_Tests}.
...
Dabei können Regelparameter variiert werden oder auch Systemparameter.
...
\subsection{Vergleiche von \xots s}

Beispielsweise kann man die Ergebnisse mit verschiedenen \ricc-Parametern \mat{Q} und $R$ vergleichen.
In \figref{fig:qrvglrib} sind die Maximalabweichungen mit verschiedenen QR-Parametern \siehe{\secref{subsec:simqr}} am neuen System dargestellt.
Insgesamt lässt sich feststellen, dass \apd\ die größten Startabweichungen noch ausregeln kann.
Die in dieser Arbeit optimierten Parameter können das \dpd\ in einem etwas größeren Bereich stabilisieren.
...
Bei den Güteparametern ist somit immer ein gewisser Kompromiss notwendig.
...
\figref{fig:qrvglribze} vergleicht daher die \ap-Regelung mit \zm\ (ZM) und \beob\ (Beob), einerseits mit den QR-Parametern \texttt{AP\_QR\_20\_neu}, sowie mit den angepassten QR-Parametern \texttt{AP\_QR\_20\_neu\_Beob} ("`Beob (neu)"').
...
Welche der Zustände für den Test ausgelenkt werden sollen, kann angegeben werden, meistens werden nur die jeweils "`kritischen"' ausgeführt.
