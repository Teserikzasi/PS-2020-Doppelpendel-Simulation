...
\begin{enumerate}
	\item \textbf{InitSymEq}: Berechnet symbolisch die \bwgl\ nach \secref{subsec:bwgl} für das \krs\ (\path{SchlittenPendelSymF}) und das \bss\ (\path{SchlittenPendelSymA}) und speichert sie als globale Variablen. Auch die Ruhelagen werden initialisiert (definiert in \path{SchlittenPendelRuhelagen}).
	...
	\item \textbf{InitSystem(SchlittenPendelParams)}: Diese Funktion erstellt die \zrm e des \spds s nach \secref{subsec:zrm}. Dafür wird die Funktion \path{SchlittenPendelNLZSR.m} genutzt, welche das \zrm\ in Abhängigkeit der \bwgl\ und der Parameter erstellt. Aus dem \krs\ wird mittels \path{sys2sfct} eine \emph{S-Function} generiert und in \path{SchlittenPendelFunc.m} gespeichert.
